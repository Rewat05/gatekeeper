...
interface FerrofluidProps {
  className?: string
  /** Defaults to 1 (not the device's native ratio) — an ambient background doesn't need retina sharpness, and this keeps the per-frame pixel cost down. */
  dpr?: number
  paused?: boolean
  colors?: string[]
  speed?: number
  ...
  mixBlendMode?: string
  /** Render every Nth frame instead of every frame — 2 halves the GPU cost with barely perceptible smoothness loss for slow ambient motion. */
  frameSkip?: number
}
...
export default function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ dpr, alpha: true, antialias: true, preserveDrawingBuffer: true })
    ...
    const loop = (t: number) => {
      ...
      frameCount++
      if (frameCount % Math.max(1, frameSkip) !== 0) return
    ...
    return () => {
      ...
      const gpuLoseContext = renderer.gl.getExtension("WEBGL_lose_context")
      gpuLoseContext?.loseContext()
    }
  }, [
    dpr,
    paused,
